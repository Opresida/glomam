...
- [x] **Acervo "Notícias Institucionais"** *(concluído — 2026-07-23)* — migração de TODAS as notícias do site oficial antigo (`glomam.org.br/noticia`, 26 páginas) para o novo site. Seção **Institucional** na Home (carrossel infinito navy), rota `/noticiasinstitucionais` (listagem) e rota por matéria `/noticiasinstitucionais/:id` (corpo + galeria com lightbox). Fotos baixadas local e convertidas p/ webp em `public/institucional/<id>/`. Componentes `Institucional.jsx`, `InstCard.jsx`, `NoticiasInstitucionais.jsx`, `NoticiaInstitucional.jsx`; dados `src/data/noticiasInstitucionais.{json,js}`; pipeline em `scripts/` (`merge`/`build`/`fetch-institucional`, idempotentes; requer `sharp`). **509 matérias, 2159 fotos (~220 MB).** A rota tem **filtro ano/mês** e visão **Trilha** (linha do tempo sinuosa — `TrilhaInstitucional.jsx`) além da Grade.
- [x] **Menu "Institucional" + Revistas** *(concluído — 2026-07-23)* — dropdown Institucional no Header (Notícias Institucionais + Revistas e Jornais; Galeria futura). Rota `/revistas` (`Revistas.jsx`) com 41 edições migradas de `glomam.org.br/jornal`: capas webp no repo, **PDFs (~1,84 GB) no Cloudflare R2** público (`glomam-revistas`, reaproveita a conta do Direito ao Ponto), base configurável via `VITE_REVISTAS_BASE`. Dados `src/data/revistas.{json,js}`; captura `scripts/fetch-revistas.mjs`.
- [x] **Galeria de Fotos** *(concluído — 2026-07-24)* — rotas `/galeria` (grade de álbuns) + `/galeria/:id` (fotos + lightbox), item no menu Institucional. Migração de `glomam.org.br/galeria`: **372 álbuns, 28.665 fotos** no **Cloudflare R2** público (`glomam-galeria`, ~2 GB em webp full+thumb). Scraping via curl (URLs diretas, sem IA). Dados `src/data/galeria.{json,js}`; pipeline `scripts/galeria-*.mjs`.
